...
& \enclose{horizontalstrike}{[id_a(\tau)]_1,[id_b(\tau)]_1,[id_c(\tau)]_1} \quad \overset{optimize}{\rightarrow} \quad id_a(x)=x, \quad id_b(x)=k_1 \cdot x, \quad id_c(x)=k_2 \cdot x
\end{aligned}$$
...
\end{aligned}$$ where Lagrange basis $L_i(x)=\prod\limits_{j=0,j \neq i}^{n-1} \frac{x-w^j}{w^i-w^j}$
$$\begin{aligned}
L_i(x) &= \left\{
	\begin{array}{l}
        1,\quad x=w^i \\
        0,\quad x \in \mathbb{H}\textbackslash\{w^i\} \\
    \end{array}
\right.
\end{aligned}$$ where $\mathbb{H}=\{1,w,w^2,...w^{n-1}\}$ is composed of roots of unity, $\mathbb{H}$ is a multiplicative subgroup, $n=2^k$
...
& - z(x) \cdot [a(x)+\beta \cdot \enclose{horizontalstrike}{id_a(x)}x + \gamma] \cdot [b(x)+\beta \cdot \enclose{horizontalstrike}{id_b(x)}k_1 \cdot x + \gamma] \cdot [c(x)+\beta \cdot \enclose{horizontalstrike}{id_c(x)}k_2 \cdot x + \gamma] = 0 \\
\\
...
Prover sends $\enclose{horizontalstrike}{[t(\tau)]_1} \boxed{[t_{low}(\tau)]_1},\boxed{[t_{mid}(\tau)]_1},\boxed{[t_{high}(\tau)]_1}$ to verifier
...
& \enclose{horizontalstrike}{id_a(\zeta),id_b(\zeta),id_c(\zeta)} \\
& \enclose{horizontalstrike}{t(\zeta)} t_{low}(\zeta),t_{mid}(\zeta),t_{high}(\zeta)
\end{aligned}$$and sends to verifier
